...
export default function DonationPage({ campaignId }: DonationPageProps) {
  ...
  // Mock checking if user is the owner (for demo purposes, assumes specific email or just a button toggle)
  const isOwner = user?.email?.address === 'owner@crowdfunding.com'; 
  ...
  if (paymentSuccess) {
    return (
      <div className="min-h-screen relative overflow-hidden flex flex-col items-center justify-center text-center">
        ...
        <Container className="flex flex-col items-center justify-center p-6 h-full">
        ...
        {!authenticated ? (
          ...
          <div className="bg-white/60 backdrop-blur-md rounded-2xl p-4 shadow-sm flex items-center justify-between border border-white/50">
             <span className="text-sm font-medium text-gray-700 truncate max-w-[200px]">
               {user?.email?.address || user?.wallet?.address || 'Connected User'}
             </span>
             {isOwner && (
               <Button 
